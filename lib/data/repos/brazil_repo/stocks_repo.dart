...
class GetAllBrazilStocks {
  ...
  Future getAllBrazilStocks() async {
...
class GetHistoricalStocksPrice {
  String symbol;
  late String url =
      'https://brapi.dev/api/quote/$symbol?range=1y&interval=1d&fundamental=false';
  ...
  Future getPriceHistory() async {
    ...
    if (respose.statusCode == 200) {
      final List result =
          jsonDecode(respose.body)['results'][0]['historicalDataPrice'];
      return result
          .map((e) => BrazilHistoricalStocksPriceModel.fromJson(e))
          .toList();
    } else {
      Timer.periodic(
        const Duration(seconds: 30),
